/**
 * 微信小程序二维码生成工具
 * 基于 kazuhikoarase/qrcode-generator 精简改造，输出到 canvas（旧版 CanvasContext）。
 * 使用方式：
 *   const drawQrcode = require('../../utils/weapp-qrcode')
 *   drawQrcode({ canvasId: 'qrcode', text: 'https://xxx', width: 480, height: 480, ctx: this })
 *     .then(() => wx.canvasToTempFilePath(...))
 */
...
QRCodeModel.createData = function (typeNumber, errorCorrectLevel, dataList) {
  ...
  while (true) {
    ...
    buffer.put(QRCodeModel.prototype.PAD0, 8)
    if (buffer.getLengthInBits() >= totalDataCount * 8) break
    buffer.put(QRCodeModel.prototype.PAD1, 8)
  }
...
function utf16To8(text) {
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    if (c >= 0x0001 && c <= 0x007F) result += text.charAt(i)
    else if (c > 0x07FF) {
...
function _getTypeNumber(text, errorCorrectLevel) {
  var length = utf16To8(text).length
  // 8bit byte 模式各级容量（粗略）取自 QR Code 规范
  ...
  var ecIndex
  switch (errorCorrectLevel) {
    case QRErrorCorrectLevel.L: ecIndex = 0; break
    case QRErrorCorrectLevel.M: ecIndex = 1; break
    case QRErrorCorrectLevel.Q: ecIndex = 2; break
    case QRErrorCorrectLevel.H: ecIndex = 3; break
    default: ecIndex = 1
  }
  for (var t = 1; t <= 40; t++) {
    if (length <= QRCodeLimitLength[t - 1][ecIndex]) return t
  }
  throw new Error('content too long')
}

// ---------- 绘制：旧版 CanvasContext ----------
function _drawToCanvas(ctx, qrCode, opts) {
  var moduleCount = qrCode.getModuleCount()
  var tileW = opts.width / moduleCount
  var tileH = opts.height / moduleCount
  // 背景
  ctx.setFillStyle(opts.background || '#ffffff')
  ctx.fillRect(0, 0, opts.width, opts.height)
  // 前景
  ctx.setFillStyle(opts.foreground || '#000000')
  for (var row = 0; row < moduleCount; row++) {
    for (var col = 0; col < moduleCount; col++) {
      if (qrCode.isDark(row, col)) {
        var x = Math.floor(col * tileW)
        var y = Math.floor(row * tileH)
 ...
 * @param {String} options.text       要编码的内容
 * @param {Number} options.width      画布宽度（px）
 * @param {Number} options.height     画布高度（px）
 * @param {Object} options.ctx        Page/Component 的 this，用于 wx.createCanvasContext 第二参数
 * @param {Number} options.correctLevel  纠错等级，默认 H
 * @param {String} options.background    背景色，默认 #ffffff
 * @param {String} options.foreground    前景色，默认 #000000
 * @returns Promise<void>  draw 完成后 resolve
 */
function drawQrcode(options) {
  return new Promise(function (resolve, reject) {
    try {
      var text = utf16To8(options.text || '')
      var correctLevel = options.correctLevel != null ? options.correctLevel : QRErrorCorrectLevel.H
      var typeNumber = _getTypeNumber(options.text || '', correctLevel)
      var qrCode = new QRCodeModel(typeNumber, correctLevel)
      qrCode.addData(text)
      qrCode.make()
      var ctx = wx.createCanvasContext(options.canvasId, options.ctx)
      _drawToCanvas(ctx, qrCode, options)
...
drawQrcode.QRErrorCorrectLevel = QRErrorCorrectLevel
